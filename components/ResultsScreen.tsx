...
const ListItem: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <li className="flex items-start gap-3 mb-4 group">
    <div className="mt-1 flex-shrink-0 w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center group-hover:bg-emerald-500 transition-colors">
      <div className="w-2 h-2 rounded-full bg-emerald-600 dark:bg-emerald-50 group-hover:bg-white transition-colors"></div>
    </div>
...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({ result, image, crop, onBack, onChatClick, onAnalyzeRequest }) => {
  ...
  // Robust Confidence Score logic
  const confidence = typeof result.confidenceScore === 'number' 
    ? (result.confidenceScore > 1 ? result.confidenceScore : Math.round(result.confidenceScore * 100))
    : 85;
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 pb-44">
      <div className="container mx-auto p-4 max-w-3xl animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12">
            <div className="md:col-span-5 relative h-64 md:h-full min-h-[300px]">
              ...
              <div className="absolute bottom-6 left-6">
                <span className={`px-5 py-2 rounded-full text-[10px] font-black text-white uppercase tracking-[0.2em] ${result.isHealthy ? 'bg-emerald-500 shadow-xl shadow-emerald-500/40' : 'bg-rose-600 shadow-xl shadow-rose-600/40'}`}>
                  {result.isHealthy ? t('healthy') : 'Detected Disease'}
                </span>
              </div>
            </div>
            
            <div className="md:col-span-7 p-8">
              <h3 className={`text-3xl font-black mb-6 leading-tight ${result.isHealthy ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'}`}>
                {result.diseaseName}
              </h3>
              ...
              <div className="mb-10">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Analysis Confidence</span>
                  <span className={`text-sm font-black ${confidenceColor}`}>{confidence}%</span>
                </div>
                <div className="bg-slate-100 dark:bg-slate-800 h-4 rounded-full overflow-hidden shadow-inner p-1">
                  <div 
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className={`flex items-center justify-center gap-3 font-black py-4 rounded-2xl transition-all shadow-lg active:scale-95 border ${isSpeaking ? 'bg-rose-600 text-white border-rose-500 animate-pulse' : 'bg-white dark:bg-slate-800 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-slate-700'}`}
                  >
                    {isLoadingSpeech ? <div className="w-5 h-5 border-3 border-current border-t-transparent rounded-full animate-spin"></div> : isSpeaking ? <StopCircleIcon className="w-6 h-6" /> : <SpeakerLoudIcon className="w-6 h-6" />}
                    <span className="text-[10px] uppercase tracking-wider">{isLoadingSpeech ? 'Preparing...' : isSpeaking ? t('stopReading') : t('readAloud')}</span>
                  </button>
                  <button onClick={handleShare} className="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-black py-4 rounded-2xl shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all">
              ...
              <button 
                onClick={onChatClick}
                className="w-full mt-4 bg-emerald-600 text-white font-black py-5 rounded-2xl shadow-xl hover:bg-emerald-700 active:scale-95 transition-all flex items-center justify-center gap-3 text-sm uppercase tracking-widest"
              >
        ...
        <div className="space-y-8">
          ...
          {!result.isHealthy && (
            <>
              ...
              <ResultCard title={t('symptoms')} icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>}>
                <ul className="space-y-1">{result.symptoms?.map((s, i) => <ListItem key={i}>{s}</ListItem>)}</ul>
              </ResultCard>
              ...
              <ResultCard title={t('remedies')} icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.673.337a4 4 0 01-2.506.331L6 15l2-2.5m1.428-1.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.673.337a4 4 0 01-2.506.331L2 10l2-2.5m10.744 3.372a4 4 0 115.714 5.714 4 4 0 01-5.714-5.714z" /></svg>}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-emerald-50 dark:bg-emerald-950/20 p-6 rounded-[2rem] border border-emerald-100 dark:border-emerald-900/40">
                    <h4 className="font-black text-emerald-800 dark:text-emerald-400 mb-4 text-xs uppercase tracking-widest">{t('organic')}</h4>
                    <ul className="space-y-1">{result.remedies?.organic.map((r, i) => <ListItem key={i}>{r}</ListItem>)}</ul>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700">
                    <h4 className="font-black text-slate-800 dark:text-slate-200 mb-4 text-xs uppercase tracking-widest">{t('chemical')}</h4>
                    <ul className="space-y-1">{result.remedies?.chemical.map((r, i) => <ListItem key={i}>{r}</ListItem>)}</ul>
                  </div>
                </div>
              </ResultCard>

              {/* RESTORED Prevention Section */}
          ...
          {/* Verification & Regional Context Links */}
          {result.groundingLinks && result.groundingLinks.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-slate-800 shadow-xl">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-600 mb-6">Verified Regional Data</h4>
              <div className="space-y-3">
                {result.groundingLinks.map((link, i) => (
                  <a key={i} href={link.uri} target="_blank" rel="noopener" className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-all border border-slate-100 dark:border-slate-700 group">
                    <span className="text-xs font-black text-slate-700 dark:text-slate-300 group-hover:text-emerald-700">{link.title}</span>
                    <svg className="w-5 h-5 text-emerald-500 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </a>
                ))}
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 z-[60] bg-white/80 dark:bg-slate-950/80 backdrop-blur-2xl border-t border-slate-100 dark:border-slate-800 flex justify-center pb-safe">
        <button 
          onClick={onBack} 
          className="w-full max-w-sm bg-slate-900 dark:bg-emerald-600 hover:bg-black dark:hover:bg-emerald-700 text-white font-black py-4 rounded-2xl shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 text-xs uppercase tracking-widest"
        >
